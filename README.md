...
- TanStack Query, small Zustand store for demo/session state
- PWA manifest, service worker, IndexedDB offline meeting queue
- OpenAI SDK abstraction with deterministic mock fallback
...
- `src/lib/ai/provider.ts` uses OpenAI when `OPENAI_API_KEY` is available server-side, otherwise returns polished deterministic drafts.
